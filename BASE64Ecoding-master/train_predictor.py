...
from molecules.predicted_model import VAEencoder_prop
...
def main():
    filename = '/data/tp/data/per_all_w2v_30(40)_new_250000.h5'
    ...
    #charset = h5f['charset'][:]
    model = VAEencoder_prop()
    length = len(data_train[0])
    charset = len(data_train[0][0])
    modelname = '/data/tp/data/model/vae_model_250000_12260707.h5'
    predictorname = '/data/tp/data/model/predictor_model_w2v_30(40)_new_250000_12260707.h5'
    print(modelname)
    if os.path.isfile(modelname):
        model.load(charset, length, modelname, predictorname, latent_rep_size=latent_dim)
    else:
    ...
    TensorBoardname = '/data/tp/data/TensorBoard/predictor_model_w2v_30(40)_new_250000_12260707'

    tbCallBack = TensorBoard(log_dir=TensorBoardname)

    print(data_train[0])
    history = model.encoder_predictor.fit(
        data_train,
        logp_train,
        shuffle=True,
        epochs=epochs,
        ...
        validation_data=(data_val, logp_val)
    )
